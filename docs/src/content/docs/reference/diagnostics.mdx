...
| `E2003` | Invalid value for "Rate" / "Burst" | `Rate` must be above zero; `Burst` must be at least 1, since each event spends a whole token. |
...
| `W3020` | Rate limiting a Server event / function has no effect | Rates limit inbound traffic only. Remove `Rate` and `Burst`. |
