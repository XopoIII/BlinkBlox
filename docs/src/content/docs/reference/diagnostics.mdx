...
| `E3005` | Reserved identifier | `StepReplication`, `SetRateLimitHandler`, `SetDecodeErrorHandler` and `SetPacketDropHandler` name module members, in the spelling `option Casing` gives them (`stepReplication` under `Camel`, `step_replication` under `Snake`), and a scope counts as a member too. A type may not take the name of one the module declares for itself, spelled `BLINK_*`. Rename the declaration. |
