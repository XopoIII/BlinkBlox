...
| `E2003` | Invalid value for option "X" | A numeric option that is not a positive whole number (`MaxInstancesPerPacket` may also be 0), a `Casing` other than `Pascal`, `Camel` or `Snake`, or an `InboundBurst` below `MaxPacketSize` or below 128. |
...
| `E3005` | Reserved identifier | `StepReplication`, `SetRateLimitHandler`, `SetDecodeErrorHandler`, `SetPacketDropHandler`, `SetListenerErrorHandler`, `GetTrafficStats` and `ResetTrafficStats` name module members, in the spelling `option Casing` gives them (`stepReplication` under `Camel`, `step_replication` under `Snake`), and a scope counts as a member too. A type may not take the name of one the module declares for itself, spelled `BLINK_*`. Rename the declaration. |
| `E3005` | "X" is the module's Limits | With `option ExportLimits`, `Limits` -- `limits` under `Camel` or `Snake` -- is a module member, and a top-level declaration of that name would replace it. Rename it. |
...
| `E3009` | Malformed range / Expected an integer / Range outside bounds | Fix the range: integer types and lengths need whole numbers, and the range must fit the type (an array holds at most 65535). A scaled number's range is in real units, and each bound must be a whole number of steps (`Bound is not a whole number of steps`). |
...
| `E3031` | Client-sent event / function "X" carries an Instance | `option MaxInstancesPerPacket = 0` refuses every packet that carries an Instance or `unknown`, and this declaration's client-sent data (or, for a `From: Server` function, the client's reply) carries one, so none of its packets could arrive. Raise the limit, or send an id instead of the Instance. In an imported file it is reported at the `import`. |
| `E3032` | Only an integer takes a scale / A scale must be greater than zero / Repeated scale / Repeated saturate / Unknown number modifier | A number's angle brackets take one [scale](/BlinkBlox/language/types/#fixed-point-numbers), a positive number on an integer type, and [`saturate`](/BlinkBlox/language/types/#saturating-on-send), each at most once. |
| `E3033` | Stream "X" is not sent by the server / is not OrderedUnreliable / cannot be polled / streams an optional / streams a type pack / streams a type that cannot be optional / has a Fast rate no faster than its Rate | A [stream](/BlinkBlox/language/streams/) is `From: Server`, `OrderedUnreliable`, heard through `On`, and streams one type that is not optional -- nil is how a clear arrives; use a struct for several values. `Fast` must be higher than `Rate`. |
| `E3034` | Event "X" is stamped but not sent by a client / Stamped event "X" cannot be polled / Stamp window of "X" is out of range / already has an element named SentAt | A [stamp](/BlinkBlox/language/stamps/) is on a `From: Client` event heard through `On`, from 1 to 30000 milliseconds, and reaches the listener as `SentAt`, so no declared element may take that name. |
...
The first five messages below are about packets the server refused whole or cut short. When the game
has installed a [packet-drop handler](/BlinkBlox/reference/generated-api/#module-members), they are
not printed: the handler is called instead, with the reason (`Malformed`, `Oversized`, `Instances`,
`Events`, `Budget`), the count and the detail.
...
| `"F" from X already has N calls running, N refused.` | server | The function's [`Concurrency`](/BlinkBlox/language/functions/#concurrency) limit refused a call; it was answered with a failure. Reported once a second per player and function, and to the rate-limit handler. |
